...
local function mode_overrides(api, name, extend)
	evloop.subscribe { -- ad-hoc registering for when user wants to subscribe
		event = 'NewHook',
		mode = name,
		pattern = 'ImgChange',
		callback = function()
	...
	local ret = {
		map = function(b, action, desc)
			...
			for _, b in ipairs(type(b) == 'table' and b or { b }) do
				...
				mappings[b] = { at = ('%s: %d'):format(i.short_src, i.linedefined), fn = action, desc = desc }
...
local function viewer_proxy(name)
	...
	self = proxy('swi.' .. name, api, mode_overrides(api, name, overrides))
...
local function gallery_proxy()
	...
	return proxy('swi.' .. name, api, mode_overrides(api, name, overrides))
end
...
local function imagelist_proxy()
	...
	local function set_mark(x, enabled, silent)
		...
		if not silent then evloop.trigger { event = 'OptionSet', match = 'swi.imagelist.marked.size', data = msize } end
	end
	...
	return proxy('swi.imagelist', api, {
...
_G.swi = proxy('swi', _s, {
	...
	---@param cmd string
	exec = function(cmd)
		cmd = cmd:gsub('([^%%])%%f', function(a) return string.format("%s'%s'", a, l.get_current().path) end)
			:gsub('([^%%])%%[sm]', function(a)
				local s = table.concat(l.marked.get(), "' '")
				return string.format("%s'%s'", a, #s > 0 and s or l.get_current().path)
			end)
			:gsub('([^%%])%%([^%%])', function(a, b) return string.format('%s%s%s', a, l.get_current().path, b) end)
			:gsub('%%%%', '%%')
...
if not rawget(swi, '_initialized') then rawset(swi, '_initialized', false) end
